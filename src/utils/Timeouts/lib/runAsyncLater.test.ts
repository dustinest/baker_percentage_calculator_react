...
describe("runAsyncLater works", () => {
  ...
  it ("can be scheduled", async () => {
    const time = await measure(async () => {
      const result = await runAsyncLater(async () => mockCallback(), 100);
  ...
  it ("schedule immediately", async () => {
    const time = await measure(async () => {
      const result = await runAsyncLater(async () => mockCallback());
      expect(result).toBe("abc");
    });
    expect(mockCallback.mock.calls.length).toBe(1);
  ...
  it ("can be cancelled", async () => {
    const time = await measure(async () => {
      const result = runAsyncLater(async () => mockCallback(), 100);
      result.cancel();
      let message = "";
      try {
        await result;
