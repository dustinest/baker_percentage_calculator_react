...
Deno.test("Bug 4: Croissant — doughGrams includes Eeltaigen (887g, not 467g)", () => {
  ...
  assertAlmostEquals(doughGrams, 887, 1, "dough weight (Eeltaigen + Taigen) should be 887g");
  assertAlmostEquals(doughGrams + customTotal, totalGrams, 1, "doughGrams + Kihistamiseks must equal Kokku");
});

Deno.test("Bug 4: Kaneelirullid — doughGrams includes Eeltaigen (907g, not 531g)", () => {
  const { doughGrams, customGroups, totalGrams } = computeSummaryWeights(bp("Kaneelirullid"));
  const customTotal = customGroups.reduce((s, g) => s + g.grams, 0);
  assertAlmostEquals(doughGrams, 907, 1, "dough weight (Eeltaigen + Taigen) should be 907g");
  assertAlmostEquals(doughGrams + customTotal, totalGrams, 1, "doughGrams + Kaanelikiht must equal Kokku");
});

Deno.test("Bug 4: Moskva saiakesed — doughGrams includes Eeltaigen (746g, not 346g)", () => {
  const { doughGrams, customGroups, totalGrams } = computeSummaryWeights(bp("Moskva saiakesed"));
  const customTotal = customGroups.reduce((s, g) => s + g.grams, 0);
  assertAlmostEquals(doughGrams, 746, 1, "dough weight (Eeltaigen + Taigen) should be 746g");
  assertAlmostEquals(doughGrams + customTotal, totalGrams, 1, "doughGrams + Kihistamiseks must equal Kokku");
});

// Sanity checks: recipes without extra groups should still work correctly
// (showDough = false, doughGrams + 0 = totalGrams).
Deno.test("Bug 4 (sanity): Vastlakuklid — no extra groups, doughGrams = totalGrams", () => {
  const { doughGrams, customGroups, totalGrams } = computeSummaryWeights(bp("Vastlakuklid"));
  assertAlmostEquals(doughGrams + customGroups.reduce((s, g) => s + g.grams, 0), totalGrams, 1);
});
